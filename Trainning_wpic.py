...
def train(model1, epoc, trainset, lr, weight_decay):  # 用于训练的函数
    ...
    train_dataloader = DataLoader(trainset, batch_size=batchsize, shuffle=True, num_workers=1, drop_last=True)
    ...
    for epoch in range(epoc):
        ...
        for data in train_dataloader:
            label, distance, theta, bs,image = data
            label, distance, theta, bs,image = label.float().to(device), distance.float().to(device), theta.float().to(
                device), bs.float().to(device),image.float().to(device)

            optimizer.zero_grad()
            outputs = model1((distance, theta, bs,image)).squeeze()

            meloss = label * PL_F - outputs * PL_F
            mseloss = torch.sqrt(MSE(outputs * PL_F,label * PL_F))

            loss_me += torch.mean(meloss)
            loss_rmse +=  torch.mean(mseloss)

            torch.sum(mseloss).backward()
            optimizer.step()
            counter += 1

        scheduler.step()
        LOSS_ME_avg = loss_me / counter
        LOSS_RMSE_avg = loss_rmse / counter
        ...
        print(f'LOSS_RMSE in epoch{epoch} is: {LOSS_RMSE_avg}')
    return model1
...
def test(testset):  # 用于测试的函数
    ...
    LOSS_RMSE_avg = loss_rmse / (counter)
    print('==========================test========================')
    print(f'RMSELoss is:{LOSS_RMSE_avg}', f'MELoss is:{LOSS_ME_avg}')
...
if __name__ == "__main__":
    ...
    torch.manual_seed(1)
    # ===================================================
    key = 'Ningbo_satellite'
    traincsv = './post-Data/training_nb_satemap.csv'
    Dataset = CustomImageDataset(traincsv)
    PL_F = Dataset.pl_s
    trainset, testset = train_test_split(Dataset, test_size=0.2, random_state=42)

    batchsize = 4
    model1 = model.Net2().cuda()
